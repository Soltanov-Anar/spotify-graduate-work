...
export const Body = ({ spotifyApi, chooseTrack }) => {
  ...
  useEffect(() => {
    ...
    spotifyApi.getNewReleases().then((releases) => {
      setNewReleases(
        releases.body.albums.items.map((track) => {
          return {
            id: track.id,
            artist: track.artists[0].name,
            title: track.name,
    ...
    <section className="bg-black ml-24 py-4 space-y-8 md:max-w-6xl flex-grow md:mr-2.5">
      ...
      <div className="flex gap-x-8 absolute min-w-full md:relative ml-6">
        ...
        <div className="">
          ...
          <div className="space-y-3 border-2 border-[#262626] rounded-2xl p-3 bg-[#0d0d0d] overflow-scroll h-[1000px] md:h-96 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-thumb-rounded hover:scrollbar-thumb-gray-500 w-[830px]">
            {searchResult.length === 0
              ? newReleases
